...
public sealed class SqliteVectorStoreTests : IDisposable
{
    ...
    public async Task SearchAsync_FindsSimilarDocuments()
    {
        ...
        var results = await store.SearchAsync(queryEmbedding, topK: 2, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task SearchAsync_RespectsTopK()
    {
        ...
        // Act
        var results = await store.SearchAsync(new float[] { 0.5f, 0.5f, 0.5f }, topK: 3, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task SearchAsync_EmptyDatabase_ReturnsEmptyList()
    {
        ...
        // Act
        var results = await store.SearchAsync(new float[] { 1.0f, 0.0f, 0.0f }, topK: 5, cancellationToken: TestContext.Current.CancellationToken);
